...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($action === 'delete_coverage' && $coverageId <= 0) {
            $message = 'Choose a valid coverage location.';
            $messageType = 'error';
        } elseif (($action === 'create_coverage' && $areaName === '') || ($action === 'update_coverage' && $coverageId <= 0)) {
        ...
        if ($message === '') {
            try {
                ...
                if ($action === 'create_coverage') {
                    ...
                    $message = 'Coverage location added.';
                    ...
                    $message = 'Coverage location updated.';
                } elseif ($action === 'delete_coverage') {
                    $pdo->prepare('UPDATE orders SET delivery_coverage_id = NULL, rider_id = NULL WHERE delivery_coverage_id = :coverage_id AND status <> "Delivered"')->execute([':coverage_id' => $coverageId]);
                    $pdo->prepare('DELETE FROM delivery_coverages WHERE id = :id')->execute([':id' => $coverageId]);
                    $message = 'Coverage location deleted.';
                }
                ...
                if (in_array($action, ['create_coverage', 'update_coverage'], true) && $riderId > 0) {
                    ...
                    $orderUpdate->execute([':rider_id' => $riderId, ':coverage_id' => $coverageId]);
                } elseif ($action === 'update_coverage') {
                ...
                $message = $exception->getCode() === '23000' ? 'That coverage location already exists.' : 'Unable to save the coverage location right now.';
                $messageType = 'error';
            }
        }
    }

    $riders = $pdo->query('SELECT id, name FROM users WHERE role = "rider" ORDER BY name ASC')->fetchAll();
    ...
    $message = 'Unable to load coverage locations right now.';
    $messageType = 'error';
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <style>
        ...
        * { box-sizing:border-box; }
        ...
        .muted { color:#4b4e4a; font-size:.88rem; }
        table { width:100%; border-collapse:collapse; }
        th, td { padding:12px 8px; border-bottom:1px solid var(--line); text-align:left; vertical-align:top; }
        th { font-size:.8rem; letter-spacing:.08em; text-transform:uppercase; color:#4b4e4a; }
...
<body>
    <div class="dashboard-shell">
        ...
        <div class="content">
            <header class="topbar">
                <div class="topbar-title"><span class="burger"><span></span><span></span><span></span></span><span>Locations</span></div>
            ...
            <main class="main-inner">
                <div class="wrap">
                    ...
                    <div class="panel">
                        ...
                        <p class="muted">Add locations and assign a delivery rider. New orders use the selected rider automatically.</p>
                        <form method="post" action="locations.php" class="coverage-form">
                            <input type="hidden" name="action" value="create_coverage" />
                            <label>Location<input type="text" name="area_name" placeholder="e.g. Downtown" required /></label>
                            <label>Delivery rider<select name="rider_id"><option value="0">Unassigned</option><?php foreach ($riders as $rider): ?><option value="<?php echo (int) $rider['id']; ?>"><?php echo htmlspecialchars($rider['name']); ?></option><?php endforeach; ?></select></label>
                        ...
                        <table>
                            <thead><tr><th>Location</th><th>Assigned rider</th><th>Status</th><th>Update</th></tr></thead>
                            <tbody>
                            <?php if (!$coverages): ?>
                                <tr><td colspan="4">No coverage locations configured yet.</td></tr>
                            <?php else: foreach ($coverages as $coverage): ?>
                                <tr>
                                    ...
                                    <td><form method="post" action="locations.php" class="coverage-form" style="grid-template-columns:1fr 130px 110px;margin:0;"><input type="hidden" name="action" value="update_coverage" /><input type="hidden" name="coverage_id" value="<?php echo (int) $coverage['id']; ?>" /><select name="rider_id" aria-label="Rider for <?php echo htmlspecialchars($coverage['area_name']); ?>"><option value="0">Unassigned</option><?php foreach ($riders as $rider): ?><option value="<?php echo (int) $rider['id']; ?>" <?php echo ((int) $coverage['rider_id'] === (int) $rider['id']) ? 'selected' : ''; ?>><?php echo htmlspecialchars($rider['name']); ?></option><?php endforeach; ?></select><select name="status" aria-label="Status for <?php echo htmlspecialchars($coverage['area_name']); ?>"><option value="active" <?php echo $coverage['status'] === 'active' ? 'selected' : ''; ?>>Active</option><option value="inactive" <?php echo $coverage['status'] === 'inactive' ? 'selected' : ''; ?>>Inactive</option></select><button type="submit">Save</button></form><form method="post" action="locations.php" style="margin-top:8px;" onsubmit="return confirm('Delete this location?');"><input type="hidden" name="action" value="delete_coverage" /><input type="hidden" name="coverage_id" value="<?php echo (int) $coverage['id']; ?>" /><button type="submit">Delete</button></form></td>
                                </tr>
